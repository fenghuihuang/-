...
// Mock Data
const MOCK_IMPLEMENTATION_DATA = [
  {
    id: 1,
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-primary/5 border-primary/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">实施项目总数</CardTitle>
            <ClipboardList className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-primary">{stats.count} 个</div>
            <p className="text-xs text-muted-foreground mt-1">当前实施库项目总数</p>
          </CardContent>
        </Card>
        <Card className="bg-primary/5 border-primary/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">实施库项目总额</p>
          </CardContent>
        </Card>
        <Card className="bg-primary/5 border-primary/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="flex justify-between items-center">
        <Tabs value={viewMode} onValueChange={(v: any) => setViewMode(v)} className="w-full md:w-auto">
          <TabsList>
            <TabsTrigger value="all">全部项目实施</TabsTrigger>
            <TabsTrigger value="mine">我的项目实施管理</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Button className="bg-primary hover:bg-primary/90" onClick={() => { setIsEdit(false); setSelectedProject(null); setIsAddEditOpen(true); }}>
          <Plus className="mr-2 h-4 w-4" /> 新增实施项目
        </Button>
      </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {MOCK_IMPLEMENTATION_DATA.map((item) => (
      ...
      <Dialog open={isAddEditOpen} onOpenChange={setIsAddEditOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle>{isEdit ? '编辑实施项目' : '新增实施项目'}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader><DialogTitle>项目实施详情</DialogTitle></DialogHeader>
          <ScrollArea className="flex-1 pr-4">
